...
@section('container')
    ...
    <div class="col-lg-12">
        <form method="POST" action="/publishers/{{ $publisher->id }}" class="mb-5">
            @csrf
            @method('put')
            <div class="mb-3">
                <label for="name" class="form-label">Name</label>
                <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $publisher->name) }}" autofocus>
                @error('name')
